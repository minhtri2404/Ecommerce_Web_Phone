<template>
    <div class="flex-1 py-10 flex flex-col justify-between">
        ...
        <h2 class="text-2xl font-semibold text-center mb-6">Quản Lý Thương Hiệu</h2>
        <!-- Search -->
                ...
                <table class="md:table-auto table-fixed w-full overflow-hidden">
                    <thead class="text-gray-900 text-sm text-left">
                        <tr>
                            ...
                            <th class="px-4 py-3 font-semibold truncate">Tên thương hiệu</th>
                    ...
                    <tbody class="text-sm text-gray-500">
                        <tr
                            v-for="brand in paginatedBrand"
                            :key="brand.id"
                            class="border-t border-gray-500/20"
                        >
                            <td class="px-4 py-3">{{ brand.sno }}</td>
                            <td class="px-4 py-3">
                                <img
                                    v-if="brand.image"
                                    :src="`http://localhost:4000/uploads/brand/${brand.image}`"
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex justify-center gap-2">
                                    <router-link
                                        :to="`/admin-dashboard/brand/${brand.id}`"
...
const fetchBrand = async() => {
   try {
        const res = await axios.get('http://localhost:4000/api/brand/')
        if (res && res.data && Array.isArray(res.data.brand)) {
            brands.value = res.data.brand.map((brand, index) => ({
                id: brand._id,
                sno: index + 1,
                name: brand.name,
                image: brand.image
...
const deleteBrand = async(id) => {
    const confirmDelete = confirm('Bạn có muốn xóa thương hiệu này không')
    if (confirmDelete) {
        try {
            const res = await axios.delete(`http://localhost:4000/api/brand/${id}`,
...
// Pagination
const totalPages = computed(() => Math.ceil(filteredBrand.value.length / itemsPerPage))
const paginatedBrand = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage
  const end = start + itemsPerPage
  return filteredBrand.value.slice(start, end)
})
